...
const ManageProforma = () => {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this customer?")) {
      try {
        await axios.delete(`http://localhost:5000/api/store_customers/${id}`);
  ...
  return (
    <section className="flex flex-col">
      <div className="pb-2 flex items-center justify-between border-b-2 mb-4 border-gray-300">
        <PageTitle pageTitle={"Proforma Invoice List"} />
        <BreadCrumbs pageName={"create-proforma"} />
      </div>
                    ...
                    <thead className="sticky top-0 bg-gray-50 z-10">
                      <tr>
                        <th
                          ...
                          onClick={() => requestSort("proma")}
                        >
                          <div className="flex items-center justify-between">
                            Proma {getSortIndicator("proma")}
                          </div>
                        </th>
                        <th
                          scope="col"
                        ...
                        <th
                          scope="col"
                          className="p-5 text-left whitespace-nowrap text-sm leading-6 font-semibold text-gray-900 capitalize min-w-[150px] cursor-pointer"
                          onClick={() => requestSort("issue-date")}
                        >
                          <div className="flex items-center justify-between">
                            Issue Date {getSortIndicator("issue-date")}
                          </div>
                        </th>
                        <th
                          scope="col"
                          className="p-5 text-left whitespace-nowrap text-sm leading-6 font-semibold text-gray-900 capitalize min-w-[150px] cursor-pointer"
                          onClick={() => requestSort("due-date")}
                        >
                          <div className="flex items-center justify-between">
                            Due Date {getSortIndicator("due-date")}
                          </div>
                        </th>
                        <th
                          scope="col"
                        ...
                        currentCustomers.map((cus, index) => (
                          ...
                          >
                            ...
                            <td className="flex p-5 items-center gap-0.5">
                              ...
                              <button
                                className="p-2 rounded-full cursor-pointer bg-white group transition-all duration-500 hover:bg-red-600 flex item-center"
        ...
        <div className="flex justify-end mt-4">
          <nav className="relative z-0 inline-flex rounded shadow-sm -space-x-px">
            <button
              ...
              className="relative transition-all duration-300 cursor-pointer hover:text-white hover:bg-blue-500 inline-flex items-center px-2 py-2 rounded border border-gray-300 bg-white text-sm font-medium text-gray-500 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Previous
            </button>
            {[...Array(totalPages)].map((_, index) => (
              <button
                ...
                className={`relative inline-flex cursor-pointer items-center px-4 py-2 border border-gray-300 bg-white text-sm font-medium ${
                  currentPage === index + 1
            ...
            <button
              ...
              className="relative duration-300 cursor-pointer hover:text-white hover:bg-blue-500 inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Next
            </button>
          </nav>
